...
function render_new_cards()
{
    ...
    foreach ($related_posts as $tmp_post_id) {
        ...
        echo '<div class="mb-0 w-full"><p class="line-clamp-2" style="color: #0b0d14;">' . strip_tags(get_field('post_content', $tmp_post_id)[0]['post_text']) . '</p></div>';
        echo '</div>';
        echo '<div class="flex items-center flex-wrap pt-4 pb-3 ">';
        echo '<a href="' . get_permalink($tmp_post_id) . '" class=" text-black-background font-semibold cursor-pointer inline-flex items-center md:mb-2 lg:mb-0 underline underline-offset-1  hover:text-primary transition duration-300 ease-in text-xs md:text-sm 2xl:text-base">Read the Article
