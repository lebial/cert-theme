...
function render_highlight_cards() {
  ...
  foreach ($brainstorm_cards as $card ) {
    echo '
    <div class="brainstorm__highlight__card max-w-xl mb-10 px-12 lg:px-0 lg:mb-0 flex flex-col items-center lg:items-start">
      <img class="w-16 h-16 mb-4" src="'.$card['icon'].'" alt="brainstorm card icon">
      <p class="font-bold mb-2 text-lg lg:text-xl text-dark-blue-background text-center lg:text-left">'.$card['title'].'</p>
      <p class="text-base lg:text-lg text-dark-blue-background text-center lg:text-left">'.$card['body'].'</p>
    </div>
    ';
  }
}
