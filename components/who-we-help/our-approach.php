...
function render_buttons($opts)
{
    foreach ($opts as $option) {
        ...
        echo '
            <button type="button" name="' . $button['button_text'] . '" class="our__approach__button text-left text-white text-2xl border-b border-gray-400 border-solid py-6 pr-10 opacity-50 transition-all duration-300 hover:opacity-100">
            ' . $button['button_text'] . '
...
?>
<section class="our__approach bg-dark-blue-background">
    <h3 class="text-white text-center py-20"><?php the_field('approach_title') ?></h3>
