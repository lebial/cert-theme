...
function render_report_buttons()
{
  ...
  foreach ($ai_buttons as $idx => $button) {
    echo '<button class="ai__driven__report__option py-4 border-gray-300  border-b text-white text-left relative line-reveal-animation
...
function render_subheader()
{
  ...
  foreach ($phraseArray as $idx => $phrase) {
    echo '<p data-animate-delay="' . ($duration + .3) * $idx . '" data-animate-duration="' . $duration . '" class="reveal-text ml-4 text-lg lg:text-2xl mb-0 text-white">' . $phrase . '.</p>';
  }
}

?>
<section class="ai__driven__section w-full px-6 mt-14">
  <h3 class="mx-auto w-fit reveal-text text-white font-bold"><?php the_field('ai_heading') ?></h3>
