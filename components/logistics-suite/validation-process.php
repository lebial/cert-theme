...
{
  $length = count($arr);
  $option = ['position' => 'last', 'nextPos' => 0, 'prevPos' => $idx - 1];
  if ($length - 1 == $idx)
    return $option;
  if ($idx == 0)
    return ['position' => 'first', 'nextPos' => $idx + 1, 'prevPos' => $length - 1];
  return ['position' => '', 'nextPos' => $idx + 1, 'prevPos' => $idx - 1];
}
function render_data_points($data)
{
  $cards = $data;
  foreach ($cards as $idx => $card) {
    $active = $idx == 0 ? 'is-card-active z-40' : '';
    $position = get_first_or_last_pos($cards, $idx);
    echo '
      <div class="logistics__point__card text-center lg:text-left flex items-center flex-col max-w-[350px] h-fit opacity-100 z-20 group flex-1 hover:cursor-pointer relative ' . $active . '"
       name="' . $position['position'] . '" data-dot-position="' . $idx . '" data-dot-next="' . $position['nextPos'] . '" data-dot-prev="' . $position['prevPos'] . '">

        <p class="text-slate-400 hidden lg:block text-xl pb-6 mb-0 font-bold group-hover:text-primary group-[.is-card-active]:text-primary transition-all duration-300 relative lg:absolute lg:-top-20 text-center">' . $card['title'] . '</p>
        <p class="text-primary block lg:hidden text-xl pb-6 mb-0 font-bold transition-all duration-300 relative lg:absolute lg:-top-20 text-center">' . $card['title'] . '</p>
    ...
    echo '
      <div class="copy__container group absolute left-1/2 -translate-x-1/2 ' . $active . '" data-copy-position="' . $idx . '">
        <div class="copy__body opacity-0 group-[.copy--active]:opacity-100 ">
          <p class="text-lg transition-all duration-300  text-dark-blue-background leading-snug text-center">
          ' . $card['description'] . '
          </p>
        </div>
      </div>
...
?>
<section class="data-ingestion__validation__process w-full h-full bg-slate-100 pt-16 pb-10">
  <h2
    class="max-w-none tracking-normal text-2xl lg:text-4xl text-dark-blue-background mx-auto font-bold text-center !mb-10 lg:!mb-40">
  ...
  <div class="w-8/12 lg:w-10/12 mx-auto relative lg:mt-24 flex flex-col lg:flex-row justify-between">
    ...
    <div class="w-full flex justify-between relative max-w-[1900px]">
      ...
      <div
        class="navigation__container hidden lg:flex justify-between w-[90vw] max-w-[1900px] absolute left-1/2 -translate-x-1/2">
        ...
        <button type="button" class="process-navigation hidden lg:block" name="next">
          <svg class="w-8" xmlns="http://www.w3.org/2000/svg" width="20" height="35" viewBox="0 0 40 69" fill="none">'
            <path fill-rule="evenodd" clip-rule="evenodd"
              d="M6.4664 1.51669C5.09958 0.149872 2.88351 0.149872 1.51669 1.51669C0.149872 2.88358 0.149872 5.09964 1.51669 6.46646L29.7027 34.6525L1.47482 62.8805C0.108002 64.2473 0.108002 66.4633 1.47482 67.8302C2.84164 69.1971 5.05771 69.1971 6.42453 67.8302L34.6521 39.6018L39.305 34.9497L39.6018 34.6521L6.4664 1.51669Z"
              fill="#7878787a" />
          </svg>
        </button>
      </div>
  ...
  <div class="copy__container mx-auto relative min-h-52 py-20 hidden lg:block">
