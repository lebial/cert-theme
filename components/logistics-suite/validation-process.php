  ...
  foreach ($cards as $idx => $card) {
    ...
    echo '
      <div class="logistics__point__card flex items-center flex-col max-w-[230px] h-fit opacity-100 z-20 group flex-1 hover:cursor-pointer relative '.$active.'" >
        <p class=" text-slate-400 text-2xl pb-6 mb-0 font-bold group-hover:text-primary group-[.is-card-active]:text-primary transition-all duration-300 absolute -top-20 text-center">'.$card['title'].'</p>
        <div class="relative ">
...
<section class="data-ingestion__validation__process w-full h-full bg-slate-100 py-16">
  <p class="max-w-none text-5xl text-dark-blue-background mx-auto text-center !mb-36"><?php the_field('section_title')?></p>
  <div class=" w-10/12 mx-auto relative mt-24 flex justify-between">
    <div class="w-10/12 bg-slate-300 h-[2px] absolute top-[24px] z-10 left-1/2 -translate-x-1/2"></div>
