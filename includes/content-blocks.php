<div class="content-blocks <?php if (!get_field('include_page_header')) {
															echo 'no-header';
														}; ?>">
	...
	<?php if (have_rows('content_block')) : while (have_rows('content_block')) : the_row(); ?>
			...
			<?php elseif (get_row_layout() == 'centered_text') : ?>
				...
				<div class="cb cb-cen-text">
					...
					<div class="contain <?php if (get_sub_field('add_border_around_text')) {
																echo 'border';
															}; ?>">
...
{
	$download_documents = get_field('download_documents', 'option');
	if ($download_documents) {
		foreach ($download_documents as $document) {
			echo '<a class="absolute w-0 h-0 hidden" download style="left: 200vw;" id="' . $document['name'] . '_case_study" href="' . $document['document'] . '"></a>';
		}
	}
	// echo '<a class="absolute w-0 h-0 hidden" download style="left: 200vw;" id="employers_case_study" href="' . get_field('employers_case_study', 'option') . '"></a>';
	// echo '<a class="absolute w-0 h-0 hidden" download style="left: 200vw;" id="advisors_case_study" href="' . get_field('advisors_case_study', 'option') . '"></a>';
}
?>
